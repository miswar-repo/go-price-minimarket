...
func GetProductsByKey(w http.ResponseWriter, r *http.Request) {
	...
	start, limit, key, store, strStore := consStart, consLimit, "", "", ""
	...
	if key == "" {
		...
		http.Error(w, "Error, mohon masukkan keyword", 500)
		bolError = true
	} else if store != "" {
	...
	if !bolError {
		...
		strQuery := fmt.Sprintf("select store_id,id,name,harga,url,Image,update_date from product "+
			" where MATCH(name) AGAINST('%s') %s ORDER BY MATCH(name) AGAINST('%s') DESC   %s %s", key, strStore, key, strLimit, strStart)
